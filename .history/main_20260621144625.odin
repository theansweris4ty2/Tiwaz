...
if k2.mouse_button_went_down(.Left) {
    for &tile in tiles {
        if check_collisions(tile.rect, point) && player.action_points1 > 0{
            tile.color = k2.RED
            if player.action_points1 
        }
    }
    for button in buttons {
        if check_collisions(button.rect, point){
            dice1_index, dice2_index, dice3_index = roll_dice() 
            player.action_points1 = dice1_index
            player.action_points2 = dice2_index
            player.action_points3 = dice3_index
...
y: f32 = 700
    ...
    fmt.println(player.action_points1)
k2.present()
}
k2.shutdown()
}
