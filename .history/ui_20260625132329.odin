...
create_ui ::proc() -> (first_buttons: [dynamic; 5]Button, main: Menu, stats: Menu, player: Player) {
    ...
    crops := fmt.tprintf("Crops: %d", player.crops)
        ore := fmt.tprintf("Ore: %d", player.ore)
        lumber := fmt.tprintf("Lumber: %d", player.lumber)
        troops := fmt.tprintf("Troops: %d", player.troops)
    ...
    action_button3:= Button{{main_menu.rect.x + 270, main_menu.rect.y + 300, 110, 50}, SLATE, ore}
    stat_button1:= Button{{stats_menu.rect.x + 50, stats_menu.rect.y + 100, 110, 50}, WHEAT_GOLD, "Farm"}
    stat_button2:= Button{{stats_menu.rect.x + 50, stats_menu.rect.y + 150, 110, 50}, FOREST_GREEN, "Log"}
    stat_button3:= Button{{stats_menu.rect.x + 50, stats_menu.rect.y + 200, 110, 50}, SLATE, "Mining"}
...
draw_ui ::proc(menu: Menu, buttons: [dynamic; 5]Button, index1: i32, index2: i32, index3: i32, action1: i32, action2: i32, action3: i32, stats: Menu){
